...
const getMatches = async (dayId) => {
    try {
        const {Stages} = (await axios
            .get(`https://prod-public-api.livescore.com/v1/api/app/date/soccer/${dayId}/7?MD=1`)).data;
        const BASE_IMG_URL = 'https://lsm-static-prod.livescore.com/medium/';
        const wc = Stages.filter((o) => o.CompId == '54' || o.CompId==108);
        const matches = [];
        const matchesIds = [];
        const teamIds = [];
        wc.forEach((w) => {
