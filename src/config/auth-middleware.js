...
const verifyCookie = (req, roles) => {
    if (roles.length > 0) {
        ...
        try {
            if (!jwt) {
                throw new Exception('Token not found', EXCEPTION_TYPES.AUTH).bind(
                    'verifyCookie=>checkJwt');
            }
            jwt = jwt.replace('Bearer ', '');
            const user = verify(jwt);
            if (
                !roles.includes(ROLE.IS_AUTHENTICATED)
          && !roles.includes(user.type)
            ) {
                throw new Exception(
                    'Not allowed to access this source',
                    EXCEPTION_TYPES.AUTH
                ).bind('verifyCookie=>checkRoles');
            }
            if (user) {
            ...
            throw new Exception(e.message, EXCEPTION_TYPES.AUTH).bind(
                'verifyCookie=>jwtValidate'
            );
        }
    }
};
